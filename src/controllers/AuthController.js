			...
			if (validation.error === null) {
				...
				if (emailCheck.length === 0) {
					...
					if (result) {
						...
						const message = {
							message:
								'Registration successful, check your email to verify email',
							data: data,
						};
						return helper.response(res, 'success', message, 201);
					}
					return helper.response(res, 'failled', 'Register Failed', 400);
				}
		...
		try {
			...
			if (validation.error == null) {
				...
				if (checkEmail.length != 0) {
					if (check.length != 0) {
						if (
							setData.code === check[0].code &&
							setData.email === check[0].email
						) {
							const data = {
								is_active: 1,
							};
						...
						return helper.response(
							res,
							'failed',
							'Activation failed, The code is wrong!',
							401
						);
					}
				...
				if (result.length > 0) {
					if (result[0].is_active === 1) {
						...
						if (verify) {
							...
							let refreshToken = token.createToken(
								result,
								process.env.JWT_KEY,
								'24h'
							);
			...
			if (result.length == 1) {
				...
				if (data.affectedRows == 1) {
					return helper.response(
						res,
						'success',
						`Email ${email} succesfull deleted`,
						200
					);
				}
			}
