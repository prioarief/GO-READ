...
module.exports = {
	...
	getAllBook: async (req, res) => {
        ...
        //     sort = 'title'
        // }
		// const sorting = (sort == 'created_at') ? 'DESC' : req.query.by
		let sorting = null
		if (sort === 'created_at') {
			sorting = 'DESC'
		...
		try {
			...
			const totals = result.length
			if (totals > 0) {
				const bookLength = await getImage()
				result.BookTotal = bookLength.length
				// console.log(bookLength.length)
				return res.status(200).json({
					length: result.BookTotal,
					data: result,
				})
				// console.log(result.BookTotal)
				// return helper.response(res, 'success' , result, 200)
			}
			return helper.response(res, 'failed', 'Data not found', 200)
		} catch (error) {
	...
	createBook: async (req, res) => {
		...
		try {
			...
			return helper.response(
				res,
				'failed',
				'Image extention is not supported',
				401
			)
		} catch (error) {
			console.log(error)
